...
SIZE = 30   # 每个小方格大小
BLOCK_HEIGHT = 20   # 游戏区的高度
BLOCK_WIDTH = 10    # 游戏区的宽度
...
def main():
    ...
    font_pos_x = BLOCK_WIDTH * SIZE + BORDER_WIDTH + 10  # 右侧信息显示区字体位置的X坐标
    ...
    while True:
        ...
        # 画游戏区域分隔线
        pygame.draw.line(screen, BORDER_COLOR, (SIZE * BLOCK_WIDTH + BORDER_WIDTH // 2, 0),
        ...
        if not game_over:
            ...
            if start:
                print_text(screen, font2, (SCREEN_WIDTH - gameover_size[0]) // 2, (SCREEN_HEIGHT - gameover_size[1]) // 2,
                           'GAME OVER', RED)
            ...
            for i in range(cur_block.start_pos.Y, cur_block.end_pos.Y + 1):
                for j in range(cur_block.start_pos.X, cur_block.end_pos.X + 1):
                    if cur_block.template[i][j] != '.':
                        pygame.draw.rect(screen, block_color, ((cur_pos_x + j) * SIZE, (cur_pos_y + i) * SIZE, SIZE, SIZE), 0)
